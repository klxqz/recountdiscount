 ...
 */
class shopRecountdiscountPluginBackendController extends waJsonController {
    ...
    public function execute() {
        ...
        $discount = shopDiscounts::calculate($data);
    ...
    private function getData($id) {
        ...
        $data['discount'] = 0; //waRequest::post('discount', 0);
        $data['tax'] = 0;
    ...
    private function validateExceedStocksCount($data, $order_id) {
        ...
        foreach ($this->getModel('order_items')->getByField(
                array('order_id' => $order_id, 'type' => 'product'), 'id'
        ) as $i) {
            if ($i['stock_id']) {
